...
public class SupersonicAirplane extends Airplane {
	...
	// 다만 시속 900km 이하에서는 일반 비행을, 그 이후에는 초음속 미행을 하므로
	// 900km 이하 구간에서는 오버라이딩 전 fly()를, 그 이후에는 재정의된 fly()를 실행합니다.
	@Override
	public void fly() {
		if(this.speed < 900) {
			super.fly(); // 900 이전 구간에서는 부모쪽에 있는 fly를 실행 이후에는 재정의된 아래 코드 실행 // 오버라이딩 전의 부모메서드 호출
